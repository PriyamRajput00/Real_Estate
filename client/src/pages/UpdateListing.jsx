...
const CreateListing = () => {
  ...
  const handleImageSubmit = () => {
    if (file.length > 0 && file.length + formData.imageUrls.length < 7) {
      ...
      setImageUploadError("Maximum number of images reached.");
      setUploading(false);
    }
  };
  ...
  const storeImage = async (file) => {
    return new Promise((resolve, reject) => {
      ...
      if (file.size > maxSize) {
        reject(
          new Error(
            `Image must be less than 2 MB. (${file.name} is ${(
              file.size /
              1024 /
              1024
            ).toFixed(2)} MB)`
  ...
  const handleSubmit = async(e) => {
    ...
    try {
        ...
        if(+formData.regularPrice < +formData.discountPrice) return setError('Regular price should be less than discount price');
        setLoading(true);
        setError(false);
